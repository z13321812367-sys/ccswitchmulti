...
- `#10` 用户环境包含 Codex Desktop 便携版 zip 时，“解锁模型菜单”还要处理找不到 Desktop 可执行文件的问题。第一次在 Codex 普通启动时点击会返回正在运行的 `Codex.exe` 路径，前端应记住该路径；用户完全退出后再次点击时要用该 portable 路径带 `--remote-debugging-port` 启动。若没有可记住路径，必须允许用户手动选择大写 `Codex.exe`，不要误用小写 CLI/app-server `codex.exe`。
- 手动选择 portable `Codex.exe` 的路径必须先交给后端校验再写入前端持久化；如果用户误选小写 CLI `codex.exe` 或不存在文件，不要把坏路径写进 `ccswitch.codexDesktopExecutablePath`，否则后续主按钮会一直复用错误路径。
